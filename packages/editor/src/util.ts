...
export const textTrim = (str: any) => str
  .replace(/^\s*/, '')
  .replace(/\s{1,}$/, ' ')
  .replace(/\s+/g, ' ')
...
export const getElementName = (node: any) => {
  ...
  if (t.isJSXMemberExpression(elementName)) {
    // @ts-expect-error ts-migrate(2339) FIXME: Property 'name' does not exist on type 'JSXMemberE... Remove this comment to see the full error message
...
export const getUuidAttr = (node: any) => node.attributes.find((node: any) => node && node.name && node.name.name === uuidName)

export const addUuidAttr = (node: any) => {
  node.attributes.push(
    // @ts-expect-error ts-migrate(2551) FIXME: Property 'jSXAttribute' does not exist on type 'ty... Remove this comment to see the full error message
